...
public class GraphicsPanel extends JPanel implements MouseListener, KeyListener {
	...
	public void paintComponent(Graphics g) {
		...
		if (rouletteChoice) {
			...
			g2.drawString("$$$ = " + String.valueOf(balance), 500, 20);
			ClassLoader cldr = this.getClass().getClassLoader();    // These five lines of code load the background picture.
			String imagePath = "images/jackpot-slot-machine-removebg-preview.png";            // Change this line if you want to use a different 
			URL imageURL = cldr.getResource(imagePath);                // background image.  The image should be saved in the
			ImageIcon image = new ImageIcon(imageURL);                // images directory.
			Image img = image.getImage();
			int imgWidth = img.getWidth(null);
			...
			if (rand1<0.14285714285*1) {
				g2.setColor(Color.black);
				g2.drawString("∑", 169, 420);
			} else if (rand1<(0.14285714285*2)) {
				g2.setColor(Color.black);
				g2.drawString("∞", 169, 420);
			} else if (rand1<(0.14285714285*3)) {
				g2.setColor(Color.black);
				g2.drawString("¥", 169, 420);
			} else if (rand1<(0.14285714285*4)) {
				g2.setColor(Color.black);
				g2.drawString("$", 169, 420);
			} else if (rand1<(0.14285714285*5)) {
				g2.setColor(Color.black);
				g2.drawString("ø", 169, 420);
			} else if (rand1<(0.14285714285*6)) {
				g2.setColor(Color.black);
				g2.drawString("π", 169, 420);
			} else if (rand1<(0.14285714285*7)) {
				g2.setColor(Color.black);
				g2.drawString("7", 169, 420);
			}
	...
	public void keyPressed(KeyEvent e) {
		...
		else if (keyCode== KeyEvent.VK_SPACE && slotsChoice==true) {
